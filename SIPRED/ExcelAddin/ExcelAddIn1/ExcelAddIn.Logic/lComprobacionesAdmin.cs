...
namespace ExcelAddIn.Logic
{
    ...
    public class lComprobacionesAdmin : aComprobacionesAdmin
    {
        public lComprobacionesAdmin(oComprobacion _Template, string accion) : base(_Template)
        {
            Template = _Template;
            if ((accion == "A") || (accion == "M"))
            {
                ...
                if (string.IsNullOrEmpty(Template.Formula) || string.IsNullOrWhiteSpace(Template.Formula)) _Messages.Add("Debe seleccionar un fórmula.");
